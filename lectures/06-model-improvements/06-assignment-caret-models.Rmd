...
lmfit = train(Price ~., data=train, method="lm", preProc = c("center", "scale"), tuneLength = 10, trControl = trc)
#plot(lmfit)
lmfit
```
...
```{r warning=FALSE}
rpartfit = train(Price ~., data=train, method="rpart", preProc = c("center", "scale"), tuneLength = 10, trControl = trc)
rpartfit
```
...
```{r warning=FALSE}
ctreefit = train(Price ~., data=train, method="ctree", preProc = c("center", "scale"), tuneLength = 10, trControl = trc)
plot(ctreefit)
ctreefit
```
