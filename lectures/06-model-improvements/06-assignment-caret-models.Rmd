...
fit.lm <- train(form, data = cars, method = "lm", metric = c("Rsquared"), trControl = ctrl)
```
...
fit.rp <- train(form , data = cars, method = "rpart", trControl = ctrl, tuneGrid = grid, metric = "Rsquared")
```
...
```{r}
plot(fit.rp, xTrans = function(x) - log(x), metric = "RMSE")
```
