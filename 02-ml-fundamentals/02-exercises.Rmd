...
```{r}
dat <- bind_rows( cars2010, cars2011, cars2012)    # combine data sets for all years and store in dat
```
...
```{r}
# ... ggplot2
qplot( y=dat$FE, x=dat$EngDispl ) #quick plot of Fuel Efficiency and Engine Displacement
qplot( y=dat$FE, x=dat$EngDispl ) + geom_hline(yintercept=naive_guess) #quick plot with mean line (i.e. naive guess)
set.seed(314) # create a random set of 314 numbers
samp <- dat %>% sample_n(10) #create a sample set samp of 10 random rows from the dat dataset
samp
gg <- samp %>%
