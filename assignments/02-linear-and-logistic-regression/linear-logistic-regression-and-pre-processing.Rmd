...
plsTune <- train(x = trainFingerprints, y = log10(trainPermeability), method = "pls", tuneGrid = expand.grid(ncomp = 1:10), trControl = ctrl)
```
...
library(ROCR)
data(GC)
GC.lm <- glm(good_bad~.,data=GC,family=binomial())
summary(GC,lm)
pred <- predict( GC.lm, type = 'response')
GC_pred <- prediction(pred, GC$good_bad)
```
