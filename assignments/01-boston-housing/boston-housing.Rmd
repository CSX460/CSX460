...
lm = lm( medv ~ ., data = BostonHousing)
lm1 = lm(medv ~ . - age - indus, data = BostonHousing)
 
summary(lm1)

ggplot(aes(x = medv, y = lm1$fitted.values), data = BostonHousing) + geom_point() + 
coord_equal(ylim = c(0, 50), xlim = c(0, 50)) + geom_abline(slope=1, intercept=0, color="red") + xlab( "Median Home Value") + ylab("Estimated Value") + theme(plot.background = element_blank())
