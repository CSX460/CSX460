...
sd_cv <- sapply(1:k, function(i) x [! x %in% x[(2*(i-1)+1):(2*i)] ]  %>% mean) %>% sd 
...
tpr =         function(y,yhat) {sum(y & (y == yhat) ) / sum(y)}
# fpr = ..      # See Example
tnr =         function(y,yhat) {sum(!y & (y == yhat) ) / sum(!y)}
sensitivity = function(y,yhat) {sum(y==yhat & y==TRUE )/sum(y==TRUE)} 
specificity = function(y,yhat) {sum(y==yhat & y==FALSE )/sum(y==FALSE)} 
recall =      function(y,yhat) {sum(y & (y == yhat) ) / sum(y)} 
precision =   function(y,yhat) {sum(yhat & (y == yhat) ) / sum(yhat)}
