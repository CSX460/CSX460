...
fit.min <- lm( FE ~ 1, fe)
stepAIC(fit.min, scope = ~ 1+ EngDispl + Transmission + NumCyl, direction = "forward")

# to get all predictors: fe  %>% names  %>%  cat(sep=" +")

stepAIC(fit.min, scope = ~ + EngDispl +NumCyl +Transmission + +AirAspirationMethod +NumGears +TransLockup +TransCreeperGear +DriveDesc +IntakeValvePerCyl +ExhaustValvesPerCyl +CarlineClassDesc +VarValveTiming +VarValveLift, direction = "forward")
